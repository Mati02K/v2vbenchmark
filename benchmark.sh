...
for run_data in all_runs_data:
    # SCENARIO-LEVEL METRICS (one value per run, not per vehicle)
    # RAFT Decision Time = LATEST order_committed timestamp (when LAST vehicle got order)
    order_committed_times = [v['timestamps_ms'].get('order_committed', 0) for v in run_data if v['timestamps_ms'].get('order_committed', 0) > 0]
    stopped_times = [v['timestamps_ms']['stopped'] for v in run_data]
    
    if order_committed_times:
        # Use MAX (latest) instead of MIN (earliest) - when last vehicle received order
