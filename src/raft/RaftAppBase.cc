...
// All methods here are transport-agnostic. The 4 pure-virtual methods (sendRaftToPeer,
...
void RaftAppBase::checkClusterTrigger()
{
    ...
    // Formation window: discoveryWaitMs + clusterFormationDelayMs (e.g. 5s total)
...
void RaftAppBase::broadcastClusterForm()
{
    ...
    data.resize(sizeof(int) + sizeof(double) + 1 + activeVehicles_.size() * sizeof(int));
    size_t off = 0;
    memcpy(data.data() + off, &myClusterId_, sizeof(int)); off += sizeof(int);
    memcpy(data.data() + off, &myClusterTimestamp_, sizeof(double)); off += sizeof(double);
...
void RaftAppBase::handleLateJoinOrder(const std::vector<uint8_t>& data, int senderId)
{
    ...
    if (myBatch_ == 0 || myBatch_ == currentBatch_) {
        // Batch 0 or we're already at our batch (e.g. clusters 0,1 cleared before we joined)
        resumeMovement();
    } else {
