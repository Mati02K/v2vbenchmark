...
- WAVE: cluster, allVehicles, multirounds lines
...
- `channel_utilization_heatmap_wave.png` — modes × vehicle counts grid, fixed colour scale
- `channel_utilization_timeseries_wave.png` — mean utilization over time per mode

`plot_animation.py` — generates two GIFs (priority dirs only, 4→8→16→20→24→32 progression):
